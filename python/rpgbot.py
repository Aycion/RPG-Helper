...
class RPGBot(object):
    ...
    def __init__(self):
        ...
        self.reglist['helloform'] = re.compile('^ *hello[, \.]? ?rover')
        self.reglist['exitform'] = re.compile('^ */kys *$')
    ...
    def process_text(self, update):
        ...
        if self.reglist['helloform'].match(message):
            ...
            if niceCount < 5:
                ...
                return 'Noice'
        elif self.reglist['exitform'].match(message):
            self.will_stop = True
            db.close()
            self.get_updates(offset=update['update_id'] + 1)
