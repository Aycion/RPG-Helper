...
class DBHelper:
    ...
    def add_char(self, char_name, player):
        ...
        # create table for the specific character with columns for info and items
        charstmt = "CREATE TABLE IF NOT EXISTS {} (char_info text, item text)".format(char)
        # create an index on the items column
        invidx = "CREATE INDEX IF NOT EXISTS itemIndex ON {} (item ASC)".format(char)
